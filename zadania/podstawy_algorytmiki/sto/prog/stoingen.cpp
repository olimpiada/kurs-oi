...
void gen_random(pair<int,int> n_r) {
    int n = n_r.first;
...
void gen_minans(triple t ) {
    int n = get<0>(t);
    int ans = get<1>(t);
    int range = get<2>(t);
    vector<int> A;
    for(int i=0; i<n; i++)
        A.push_back(rng(1,range));
    int k = rng(0,n-ans);
    sort(A.begin()+k,A.begin()+(k+ans));
    for(int i=1; i<ans; i++)
        if (A[k+i]<=A[k+i-1])
            A[k+i] = A[k+i-1]+1;
    cout << n << '\n';
...
//    gen_test_reseed("1ocen", gen_1ocen);
    ...
    gen_test_reseed(current_test++, gen_random, pair{1, 100});
    gen_test_reseed(current_test++, gen_random, pair{10, 10});
    gen_test_reseed(current_test++, gen_random, pair{23, 100});
    gen_test_reseed(current_test++, gen_random, pair{49, 87});

    current_test.advance_group();
    gen_test_reseed(current_test++, gen_random, pair{491, 101});
    gen_test_reseed(current_test++, gen_random, pair{999, 10'000});
    gen_test_reseed(current_test++, gen_minans, triple{1'000, 724, 100'000});
    gen_test_reseed(current_test++, gen_random, pair{1'000, 100'000});

    current_test.advance_group();
    gen_test_reseed(current_test++, gen_random, pair{999'999, 1'000'000'000});
    gen_test_reseed(current_test++, gen_random, pair{1'000'000, 1'000'000'000});
    gen_test_reseed(current_test++, gen_minans, triple{1'000'000, 333'334, 1'000'000'000});
    gen_test_reseed(current_test++, gen_minans, triple{1'000'000, 1'000'000, 1'000'000'000});
    gen_test_reseed(current_test++, gen_rev, pair{1'000'000, 1'000'000'000});
}
